...
function WeatherFutureCast() {
  ...
  const [cityLastRequestWeatherFutureCastTime, setCityLastRequestWeatherFutureCastTime] = useLocalStorage('cityLastRequestWeatherFutureCastTime', new Map())
  const [cityLastRequestWeatherFutureCastData, setCityLastRequestWeatherFutureCastData] = useLocalStorage('cityLastRequestWeatherFutureCastData', new Map())
  ...
  function resetCacheWeatherFutureCast({latitude, longitude}) {
    const data = cityLastRequestWeatherFutureCastData[`${latitude}-${longitude}`]
    setupWeatherFutureCastDisplayData({latitude, longitude}, data)
  }

  async function setupWeatherFutureCastDisplayData({latitude, longitude}, data) {
    ...
    setfutureSecondDay({day: ((todayDay+2)%7),
      ...
      setCityLastRequestWeatherFutureCastData(cityLastRequestWeatherFutureCastData)
      cityLastRequestWeatherFutureCastTime[`${latitude}-${longitude}`] = new Date()
      setCityLastRequestWeatherFutureCastTime(cityLastRequestWeatherFutureCastTime)
  }
  ...
  function getCityLastRequestWeatherFutureCastTime({latitude, longitude}) {
    const lastRequestWeatherFutureCastTime = cityLastRequestWeatherFutureCastTime[`${latitude}-${longitude}`]
    return lastRequestWeatherFutureCastTime ? lastRequestWeatherFutureCastTime : 0
  }

  useEffect(() => {
    const lastRequestWeatherFutureCastTime = getCityLastRequestWeatherFutureCastTime(weatherContext)
    const gapNewsGetTime = new Date(new Date() - new Date(lastRequestWeatherFutureCastTime))
