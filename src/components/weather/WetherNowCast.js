...
function WeatherNowCast() {
  ...
  const [weatherDescription, setWeatherDescription] = useState(
    weatherDescriptionImage('clear', new Date()).description
  )
  ...
  const [feelsLikeTemperatureUnits, setFeelsLikeTemperatureUnits] = useState(
    weatherContext.temperatureUnits
  )
  const [displayCelsius, setDisplayCelsius] = useState(
    weatherContext.temperatureUnits === convertTemperatureUnits('C')
  )
  ...
  const [surfaceShortwaveRadiation, setSurfaceShortwaveRadiation] =
    useState('0 w/sqm')
  ...
  function setRenderTemperature(sourceTemperature, sourceTemperatureUnits) {
    if (displayCelsius) {
      ...
      setTemperature(
        sourceTemperatureUnits.toLowerCase() === 'c'
          ? sourceTemperature
          : convertTemperature(sourceTemperature, true)
      )
    } else {
  ...
  function setRenderFeelsLikeTemperature(
    sourceTemperature,
    sourceTemperatureUnits
  ) {
    if (displayCelsius) {
    ...
    fetch(`${WEATHERSERVICE}${DEFINEDWEATHERNOWCAST}/${latitude}/${longitude}`)
      ...
      .then(async (data) => {
        ...
        setWeatherDescription(
          weatherDescriptionImage(data.weather_code.value, informationTime)
            .description
        )
        setRenderTemperature(+data.temp.value, data.temp.units)
        setRenderFeelsLikeTemperature(
          +data.feels_like.value,
          data.feels_like.units
        )
        setSunrise(
          localeDateTime(new Date(data.sunrise.value), offSetSeconds / 3600)
        )
        setSunset(
          localeDateTime(new Date(data.sunset.value), offSetSeconds / 3600)
        )
        ...
        setPrecipitationValue(
          `${data.precipitation.value} ${data.precipitation.units}`
        )
        setHumidty(`${data.humidity.value} ${data.humidity.units}`)
        setVisibility(`${data.visibility.value} ${data.visibility.units}`)
        setSurfaceShortwaveRadiation(
          `${data.surface_shortwave_radiation.value.toFixed(2)} ${
            data.surface_shortwave_radiation.units
          }`
        )
        setUV(convertUV(data.uv.value))
        setWindDirection(
          `${data.wind_direction.value} ${data.wind_direction.units}`
        )
        setWindGust(`${data.wind_gust.value} ${data.wind_gust.units}`)
        setWindSpeed(`${data.wind_speed.value} ${data.wind_speed.units}`)
        setObservationTime(
          localeDateTime(
            new Date(data.observation_time.value),
            offSetSeconds / 3600
          )
        )
      })
  ...
  function handleTemperatureConvert(isToFahrenheit) {
    if (
      (isToFahrenheit && temperatureUnits === convertTemperatureUnits('F')) ||
    ...
    setFeelsLikeTemperature(
      convertTemperature(feelsLikeTemperature, isToFahrenheit)
    )
    ...
    setWeatherContext({
      ...weatherContext,
      temperatureUnits: convertTemperatureUnits(newUnits),
    })
  }
      ...
      <div className='weatherNowSession'>
          ...
          <div>
            <div className='weatherImage'>
              ...
              >
                <Image
                  ...
                  alt={weatherCode}
                  data-tip
                  data-for='observationTimeToolTip'
                />
              </OverlayTrigger>
            ...
            <div>
              ...
              <ButtonGroup size='sm' toggle={true}>
                <Button
                  active={displayCelsius}
                  onClick={() => handleTemperatureConvert(false)}
                >
                  {convertTemperatureUnits('C')}
                </Button>
                <Button
                  active={!displayCelsius}
                  onClick={() => handleTemperatureConvert(true)}
                >
                  {convertTemperatureUnits('F')}
                </Button>
              </ButtonGroup>
            </div>
          </div>
        ...
        <div className='weatherNowcast'>
          <div className='weatherDescriptionFirstSession'>
            <div>
              Feels Like: {feelsLikeTemperature.toFixed(2)}
              {convertTemperatureUnits(feelsLikeTemperatureUnits)}
            </div>
            <div>
              {(() => {
                return HIDESUNRISEINFORMAITON
            ...
            <div>SW radiation: {surfaceShortwaveRadiation}</div>
            <div>UV: {uv}</div>
          </div>
