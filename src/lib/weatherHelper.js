...
const LOCATIONERVICE =
  'https://yjymxw64uayrr4a6.anvil.app/CQ5QZK23NH3UZY7HIQCUN45R/_/api/location/time-zone/'
...
async function requestUpdateLocationInformationCache(latitude, longitude) {
  ...
  CacheLocationInformation.set(`${latitude},${longitude}`, {
    createTime: new Date(),
    resource: body,
  })
}
...
async function locationInformation(latitude, longitude) {
  ...
  if (
    CacheLocationInformation.has(key) &&
    isValidDateTime(CacheLocationInformation.get(key).resource.formatted)
  ) {
    const locationInformation = CacheLocationInformation.get(key)
    const gapNewsGetTime = new Date(
      new Date() - new Date(locationInformation.createTime)
    )
...
export function localeDateTime(dateTimeUTC, offSet) {
  ...
  const offSetDateTime = new Date(
    dateTimeUTC.getTime() + tzDifference * 60 * 1000
  )
  ...
  switch (weatherCode) {
    ...
    case 'partly_cloudy':
      return {
        ...
        image: isNowDay(currentTime) ? partly_cloudy_day : partly_cloudy_night,
      }
    case 'mostly_clear':
      return {
        description: 'Mostly clear',
        image: isNowDay(currentTime) ? mostly_clear_day : mostly_clear_night,
      }
    case 'clear':
      return {
        description: 'Clear, sunny',
        image: isNowDay(currentTime) ? clear_day : clear_night,
      }
    default:
